...
namespace Railgun.Editor.App.Util
{
    ...
    static class ShapeBatch
    {
        ...
        public static Vector2 Line(Vector2 startPos, float length, float angle, float width, Color color)
        {
            ...
            endPos += new Vector2((float)Math.Cos(-angle), (float)Math.Sin(-angle)) * length;
        ...
        public static void Circle(Vector2 center, float radius, int segments, float rotation, Color colorCenter, Color colorEdge)
        {
            ...
            for (int i = 0; i < segments; i++)
            {
                ...
                Vector2 pos0 = center + new Vector2((float)Math.Cos(a0), (float)Math.Sin(a0)) * radius;
                Vector2 pos1 = center + new Vector2((float)Math.Cos(a1), (float)Math.Sin(a1)) * radius;
        ...
        public static void Circle(Vector2 center, float radius, Color colorCenter, Color colorEdge)
        {
            ...
            float angle = (float)Math.Asin(DefaultCircleSegmentLength / radius);
            int segments = (int)(Math.PI * 2.0f / angle);
        ...
        public static void Triangle(Vector2 center, float height, float rotation, Color color0, Color color1, Color color2)
        {
            ...
            float rad30 = (float)Math.PI / 6.0f;  // 30 degrees in radians
            float tan30 = (float)Math.Tan(rad30); // Tangent of 30 degrees
            ...
            float topAngle = -(float)Math.PI / 2 + rotation;
            float brAngle = rad30 + rotation;
            float blAngle = rad30 * 5 + rotation;

            // Offsets to vertices from center
            Vector2 topOffset = new Vector2((float)Math.Cos(topAngle), (float)Math.Sin(topAngle)) * centerToTop;
            Vector2 brOffset = new Vector2((float)Math.Cos(brAngle), (float)Math.Sin(brAngle)) * centerToTop;
            Vector2 blOffset = new Vector2((float)Math.Cos(blAngle), (float)Math.Sin(blAngle)) * centerToTop;
        ...
        private static void BatchWideLine(Vector2 p0, Vector2 p1, float width, Color color0, Color color1)
        {
            ...
            width = (float)Math.Max(1.0f, width);
